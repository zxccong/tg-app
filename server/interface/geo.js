...
router.get('/getPosition', async (ctx) => {
  let {
    ...
    //请求作者写的根据ip获取省份和城市名字
  // } = await axios.get(`http://cp-tools.cn/geo/getPosition?sign=${sign}`)//不用签名了
...
router.get('/province', async (ctx) => {
  ...
  //导入数据到本地数据库
  // await Province.collection.insert(province)
  ctx.body = {
    province: status === 200
      ? province
      : []
...
router.get('/city', async (ctx) => {
  ...
  if (status === 200) {
    // await City.collection.insert(city)
    ctx.body = {
      city
    }
  } else {
...
router.get('/menu', async (ctx) => {
  ...
  if (status === 200) {
    // await Menu.collection.insert(menu)
    ctx.body = {
      menu
    }
  } else {
